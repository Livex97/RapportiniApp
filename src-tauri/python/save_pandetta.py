...
def main():
    ...
    # Gestisce i duplicati: tiene solo la prima occorrenza per ogni RIF
    existing_keys = {}
    example_row = None
    for r_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=False), start=2):
        if rif_col_idx <= len(row):
            ...
            if key_cell.value is not None:
                ...
                if key_str:
                    # Se l'ID esiste già, questa è un duplicato - lo saltiamo
    ...
    for r_idx in range(2, ws.max_row + 1):
        ...
        if any(v is not None and str(v).strip() for v in row_vals):
            ...
            # Salta duplicati: usa solo la prima occorrenza di ogni RIF
